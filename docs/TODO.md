...
- [x] T269 (P0) Condition A truncation policy: if context exceeds model cap, truncate + LOG the truncation (it is itself a result)
- [x] T270 (P0) Tests: deterministic context assembly, truncation logging, token estimate
...
### graph-guided debugging case (EX04 §5.3–5.4) — concrete bug → fix
- [x] T530 (P0) `tests/fixtures/buggy_case/` — small planted-bug target (HTTP byte-range off-by-one); fixed `parser.py` + `parser_buggy.py` "before" + spec `tests/test_range.py` + README (target-choice rationale)
- [x] T531 (P0) `services/debug/case.py` + `sdk/debug_ops.py` + `hw4 debug` CLI — reproduce (red) → graph-localize via `tested_by` edge → verify fix (green) → `results/BUG_ANALYSIS.md`
- [x] T532 (P0) Bug-analysis report: problem, root cause (off-by-one, inclusive RFC 9110 range), graph-guided research path, before/after diff, verification, **token comparison 560→273 (51% saved)**
- [x] T533 (P0) `tests/unit/test_debug_case.py` (reproduce, localize, fix verified, spec red/green) + CLI dispatch test; README "Debugging case" + FR-13

### vault hot.md (EX04 §5.1) — was missing
- [x] T534 (P0) `VaultBuilder.write_hot` + `operations._write_hot` — machine-owned `hot.md` focused-context page for the rank-1 bottleneck; linked from `index.md` "Start here"; live werkzeug `hot.md` regenerated; test added

### GRAPH_REPORT.md (EX04 §7 deliverable — literal Graphify-parity output)
- [x] T535 (P0) `services/graph_report.py` — human-readable audit (nodes/edges/evidence breakdown, top bottlenecks, largest communities); emitted beside graph.json on every `hw4 graph` build; live werkzeug `results/graphs/i00/GRAPH_REPORT.md` generated; test added

### critical re-audit vs official EX04 PDF — gaps found + closed (2026-06-16)
- [x] T536 (P0) §5.3 — wire a real **graph-guided debug agent** (`hw4 debug --agent`): localize via graph → hand the CrewAI analyst ONLY the implicated module snippet → root-cause narrative (`agent_debug.md`, ledger `agent.analyst`); corrected the inaccurate "analyst narrates" claim in BUG_ANALYSIS.md + README
- [x] T537 (P0) §5.5 — completed the bug token comparison's four dimensions (tokens 51%, files 3→1, research rounds, speed/quality to root cause)
