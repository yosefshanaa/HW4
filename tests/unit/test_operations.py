"""Integration tests for sdk operations — vault build + ask/analyze on mini_repo.

The shared scaffolding (FULL_SETUP, FAST_RATE_LIMITS, make_sdk, MINI_REPO) lives
here and is reused by sibling suites (test_operations_fix, test_agents,
test_concurrency, test_confusion, test_debug_case) via `from .test_operations`.
The fix/experiment/budget suites live in test_operations_fix to keep both files
within the 150-code-line limit (guidelines §3.2/§6.1).
"""
